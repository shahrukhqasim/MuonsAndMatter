...
def main(design, output_file='plots/detector_visualization.png', params_file=None):
    ...
    def simulate_muon_(muon_data, *args):
        simulate_muon(*args)
        data = collect()
        print(data)
        muon_data += [data]
    ...
    simulate_muon_(muon_data,0, 0, 100, -1, np.random.normal(0, 0.4), np.random.normal(0, 0.4), zpos)
    simulate_muon_(muon_data, 0, 0, 50, -1, np.random.normal(0, 0.4), np.random.normal(0, 0.4), zpos)
    simulate_muon_(muon_data,0, 0, 70, +1, np.random.normal(0, 0.4), np.random.normal(0, 0.4), zpos)
    #
